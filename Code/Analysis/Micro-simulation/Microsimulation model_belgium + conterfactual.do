...
capture log close
log using "Finished\MicrosimulationModel\log_microsimulation_belgium.log", replace text
...
//==============================================================================
//svy: tab inheritor
//svy: tab inheritor if owner_status == 2 & value_2022 > 1000000
//set seed 12345
//gen random_draw = runiform() if inheritor == 1
//gen heir_type = 1 if inheritor == 1 & random_draw <= 0.80
//replace heir_type = 2 if inheritor == 1 & random_draw > 0.80 & random_draw <= 0.95
//replace heir_type = 3 if inheritor == 1 & random_draw > 0.95
//svy: tab inheritor if heir_type == 1 & value_2022 > 1000000
//svy: tab inheritor if heir_type == 2 & value_2022 > 100000
//svy: tab inheritor if heir_type == 3 


// ====================================================================================
//
//  BELGIUM REGIME (https://fin.belgium.be/en/private-individuals/death/inheritance-tax)
//
// ====================================================================================
* Assumptions:
* 1. The tax law of the Brussels-Capital Region is applied.
* 2. All heirs are children and receive a €15,000 exemption.
* 3. Two scenarios are run to account for uncertainty about the tax rate.

//==============================================================================
// 1. SETUP AND CONSTANTS
//==============================================================================

// --- Define Belgian (Brussels) tax parameters for 2022 ---
scalar bel_allowance = 15000

// Brackets are the same for both scenarios
scalar bel_bracket1 = 50000
scalar bel_bracket2 = 100000
scalar bel_bracket3 = 175000
scalar bel_bracket4 = 250000
scalar bel_bracket5 = 500000
...
// --- Calculate the common taxable base after the allowance ---
gen taxable_amount_bel = 0
replace taxable_amount_bel = . if owner_status != 2
replace taxable_amount_bel = value_2022 - bel_allowance if owner_status == 2
replace taxable_amount_bel = 0 if taxable_amount_bel < 0

//==============================================================================
// 3. SCENARIO A: REDUCED RATE (ASSUMES MAIN RESIDENCE)
//==============================================================================

// STEP 3.1: Calculate tax due with REDUCED rates
gen tax_due_bel_a = 0
replace tax_due_bel_a = . if owner_status != 2

replace tax_due_bel_a = taxable_amount_bel * 0.02 if taxable_amount_bel > 0 & taxable_amount_bel <= bel_bracket1
replace tax_due_bel_a = (bel_bracket1 * 0.02) + (taxable_amount_bel - bel_bracket1) * 0.053 if taxable_amount_bel > bel_bracket1 & taxable_amount_bel <= bel_bracket2
replace tax_due_bel_a = (bel_bracket1 * 0.02) + (bel_bracket2 - bel_bracket1) * 0.053 + (taxable_amount_bel - bel_bracket2) * 0.06 if taxable_amount_bel > bel_bracket2 & taxable_amount_bel <= bel_bracket3
replace tax_due_bel_a = (bel_bracket1 * 0.02) + (bel_bracket2 - bel_bracket1) * 0.053 + (bel_bracket3 - bel_bracket2) * 0.06 + (taxable_amount_bel - bel_bracket3) * 0.12 if taxable_amount_bel > bel_bracket3 & taxable_amount_bel <= bel_bracket4
replace tax_due_bel_a = (bel_bracket1 * 0.02) + (bel_bracket2 - bel_bracket1) * 0.053 + (bel_bracket3 - bel_bracket2) * 0.06 + (bel_bracket4 - bel_bracket3) * 0.12 + (taxable_amount_bel - bel_bracket4) * 0.24 if taxable_amount_bel > bel_bracket4 & taxable_amount_bel <= bel_bracket5
replace tax_due_bel_a = (bel_bracket1 * 0.02) + (bel_bracket2 - bel_bracket1) * 0.053 + (bel_bracket3 - bel_bracket2) * 0.06 + (bel_bracket4 - bel_bracket3) * 0.12 + (bel_bracket5 - bel_bracket4) * 0.24 + (taxable_amount_bel - bel_bracket5) * 0.30 if taxable_amount_bel > bel_bracket5

label var tax_due_bel_a "Tax Due under Belgian Regime A"

// STEP 3.2: Calculate ADDITIONAL tax
gen additional_tax_bel_a = max(0, tax_due_bel_a - tax_due_ita)
label var additional_tax_bel_a "Additional Tax Due under Belgian Regime A Simulation"
...
svy: total additional_tax_bel_a
scalar total_revenue_a = e(b)[1,1]

// STEP 4.2 A: Create the post-reform wealth variable
gen net_wealth_posttax_a = net_wealth - additional_tax_bel_a
...
esttab matrix(G, fmt(%18.6fc)) using "Finished/Tables/Microsimulation/Progressive Regimes/belgiumA1.tex", replace label booktabs
...
matrix rownames G = "Gini Coefficient" "GE(2)" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Gini (Non-Inheritors)" "Gini (Inheritors)" "Within-Group Component (GE_W)" "Between-Group Component (GE_B)"
matlist G, format(%18.6fc)
esttab matrix(G, fmt(%18.6fc)) using "Finished/Tables/Microsimulation/Progressive Regimes/belgiumA2.tex", replace label booktabs
...
matrix rownames G = "Gini Coefficient" "GE(2)" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Gini (Non-Inheritors)" "Gini (Inheritors)" "Within-Group Component (GE_W)" "Between-Group Component (GE_B)"
matlist G, format(%18.6fc)
esttab matrix(G, fmt(%18.6fc)) using "Finished/Tables/Microsimulation/Progressive Regimes/belgiumA3.tex", replace label booktabs
...
matrix rownames G = "Gini Coefficient" "GE(2)" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Gini (Non-Inheritors)" "Gini (Inheritors)" "Within-Group Component (GE_W)" "Between-Group Component (GE_B)"
matlist G, format(%18.6fc)
esttab matrix(G, fmt(%18.6fc)) using "Finished/Tables/Microsimulation/Progressive Regimes/belgiumA4.tex", replace label booktabs
...
//==============================================================================
// 9. SCENARIO B: STANDARD RATE (ASSUMES NOT MAIN RESIDENCE)
//==============================================================================

// STEP 9.1: Calculate tax due with STANDARD rates
gen tax_due_bel_b = 0
replace tax_due_bel_b = . if owner_status != 2

replace tax_due_bel_b = taxable_amount_bel * 0.03 if taxable_amount_bel > 0 & taxable_amount_bel <= bel_bracket1
replace tax_due_bel_b = (bel_bracket1 * 0.03) + (taxable_amount_bel - bel_bracket1) * 0.08 if taxable_amount_bel > bel_bracket1 & taxable_amount_bel <= bel_bracket2
replace tax_due_bel_b = (bel_bracket1 * 0.03) + (bel_bracket2 - bel_bracket1) * 0.08 + (taxable_amount_bel - bel_bracket2) * 0.09 if taxable_amount_bel > bel_bracket2 & taxable_amount_bel <= bel_bracket3
replace tax_due_bel_b = (bel_bracket1 * 0.03) + (bel_bracket2 - bel_bracket1) * 0.08 + (bel_bracket3 - bel_bracket2) * 0.09 + (taxable_amount_bel - bel_bracket3) * 0.18 if taxable_amount_bel > bel_bracket3 & taxable_amount_bel <= bel_bracket4
replace tax_due_bel_b = (bel_bracket1 * 0.03) + (bel_bracket2 - bel_bracket1) * 0.08 + (bel_bracket3 - bel_bracket2) * 0.09 + (bel_bracket4 - bel_bracket3) * 0.18 + (taxable_amount_bel - bel_bracket4) * 0.24 if taxable_amount_bel > bel_bracket4 & taxable_amount_bel <= bel_bracket5
replace tax_due_bel_b = (bel_bracket1 * 0.03) + (bel_bracket2 - bel_bracket1) * 0.08 + (bel_bracket3 - bel_bracket2) * 0.09 + (bel_bracket4 - bel_bracket3) * 0.18 + (bel_bracket5 - bel_bracket4) * 0.24 + (taxable_amount_bel - bel_bracket5) * 0.30 if taxable_amount_bel > bel_bracket5

label var tax_due_bel_b "Tax Due under Belgian Regime B"

// STEP 9.2: Calculate ADDITIONAL tax
gen additional_tax_bel_b = max(0, tax_due_bel_b - tax_due_ita)
label var additional_tax_bel_b "Additional Tax Due under Belgian Regime B Simulation"
...
svy: total additional_tax_bel_b
scalar total_revenue_b = e(b)[1,1]

// STEP 10.2 B: Create the post-reform wealth variable
gen net_wealth_posttax_b = net_wealth - additional_tax_bel_b
...
matrix rownames G = "Gini Coefficient" "GE(2)" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Gini (Non-Inheritors)" "Gini (Inheritors)" "Within-Group Component (GE_W)" "Between-Group Component (GE_B)"
matlist G, format(%18.6fc)
esttab matrix(G, fmt(%18.6fc)) using "Finished/Tables/Microsimulation/Progressive Regimes/belgiumB1.tex", replace label booktabs
...
matrix rownames G = "Gini Coefficient" "GE(2)" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Gini (Non-Inheritors)" "Gini (Inheritors)" "Within-Group Component (GE_W)" "Between-Group Component (GE_B)"
matlist G, format(%18.6fc)
esttab matrix(G, fmt(%18.6fc)) using "Finished/Tables/Microsimulation/Progressive Regimes/belgiumB2.tex", replace label booktabs
...
matrix rownames G = "Gini Coefficient" "GE(2)" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Gini (Non-Inheritors)" "Gini (Inheritors)" "Within-Group Component (GE_W)" "Between-Group Component (GE_B)"
matlist G, format(%18.6fc)
esttab matrix(G, fmt(%18.6fc)) using "Finished/Tables/Microsimulation/Progressive Regimes/belgiumB3.tex", replace label booktabs
...
matrix rownames G = "Gini Coefficient" "GE(2)" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Population Share" "Income/Wealth Share" "Relative Income/Wealth" "Gini (Non-Inheritors)" "Gini (Inheritors)" "Within-Group Component (GE_W)" "Between-Group Component (GE_B)"
matlist G, format(%18.6fc)
esttab matrix(G, fmt(%18.6fc)) using "Finished/Tables/Microsimulation/Progressive Regimes/belgiumB4.tex", replace label booktabs
